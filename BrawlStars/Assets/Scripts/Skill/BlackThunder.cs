...
public class BlackThunder : Skill
{
	public float reach;
	public float radius;
	public float radiusPerEffect;
	...
	IEnumerator DamageCoroutine()
	{
		...
		for (int i = 0; i < damageCount; i++)
		{
			...
			float distance = Random.Range(0, radius - radiusPerEffect);
			Vector3 moveVector = Quaternion.Euler(0, angle, 0) * new Vector3(1, 0, 0) * distance;
			Vector3 effectPosition = transform.position + moveVector;
			
			GameObject effectObject = ObjectPool.GetInstance().GetObject(effect.gameObject);
			effectObject.transform.position = effectPosition;
			List<Actor> targets = BattleManager.GetInstance().FindActorsInCircle(effectPosition, radiusPerEffect);
